...
public class WebRTCManager implements ISignalingEvents {
    ...
    public void joinRoom(Context context, EglBase eglBase, Intent captureIntent) {
        if (_peerHelper != null) {
            _peerHelper.initContext(context, eglBase, captureIntent);
        }
        if (_webSocket != null) {
            _webSocket.joinRoom(_roomId);
        }
